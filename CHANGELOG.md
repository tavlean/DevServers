...
- **Every menu icon renders sharper.** The bundled glyphs carried no intrinsic size, so Raycast rasterized them at 20 pixels and scaled them up for the menu, which thickened strokes and closed the gaps that give an icon its shape. They now carry a 320-pixel canvas, so the menu gets a clean downscale instead of a blurry enlargement.
