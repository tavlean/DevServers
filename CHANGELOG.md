...
- **Every menu icon now draws completely and sharply.** Raycast's SVG renderer skips `line`, `polyline`, and `polygon` elements, so Copy Port and Open in Terminal had no icon at all, the trash was missing its lid, and the Restart arrows lost their heads; all glyphs are now built from paths, which render fully. The glyphs also carried no intrinsic size, so Raycast rasterized them at 20 pixels and scaled up for the menu, blurring and thickening everything; they now carry a 320-pixel canvas, so the menu gets a clean downscale instead.
