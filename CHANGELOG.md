...
- **Every server you start gets a row the moment you confirm it.** A new **Starting** section at the top of the dashboard lists each one by project name with a spinner, and the cursor moves to it so it stays under your hands as it turns into the real server row. Starting several at once shows several rows, so you can watch them come up one by one instead of guessing behind a single toast.
- **The dashboard stays open when a server opens in your browser.** Auto-open used to bring the browser to the front, and Raycast hides itself whenever it loses focus, so the window vanished a second or two after a start. It looked like a crash, and it took away everything else you might have wanted right then: copying the URL for a different browser, opening the project in a terminal. The tab now opens behind the dashboard. Opening a URL yourself still switches to the browser, which is the point of pressing return.
...
- **The list is now ordered newest first.** Previously it came out in whatever order the system reported the processes, which is process-ID order: that only loosely tracks start time and wraps around, so a server you started an hour ago could sit above one you started seconds ago. Now the most recently started server is at the top, and starting a server moves its project up with it. It also means a starting row hands off to a server row right where you were already looking.
- **Restarting a server now shows the same row, saying "Restarting…".** It appears before the old server is stopped, so the project never blinks out of the list, and the cursor rides it across onto the replacement instead of being left on a server that no longer exists. When the server comes back it sits at the top of its project, and its project moves to the top of the list. A restart that never comes back turns red in place with the same explanation and the same actions as a failed start, rather than a toast you have probably already dismissed.
- **Fixed: Cloudflare and Wrangler projects showed phantom servers, and restarting one made copies.** A SvelteKit or Wrangler project runs `workerd` under the dev server, and those helpers were appearing as if they were dev servers of their own, on meaningless ports like `51759`. Restarting one did the only thing it could: killed the helper and started a *second* dev server for that folder, leaving the first running. Do it a few times and the project collected servers, each on the next port up. The extension already tried to hide helpers by spotting that their parent was a real server, but `workerd` outlives the process that launched it and gets adopted by the system, which erased the family link. Helpers are now recognised by sharing a folder with a real server as well, so they stay hidden however they were started.
- **Fixed: starting from the menu bar never opened the browser**, even with "open its URL automatically" turned on. Starting the very same project from the dashboard did. The setting was being read in two places and passed around, so the menu bar could hand the dashboard a different answer than the one you had actually chosen. There is now one copy of it, read where the browser is opened. One thing to check after updating: the July 9 release moved this setting to the extension level, which reset it to off for anyone who had enabled it before then. If auto-open has felt broken since, open the extension settings and turn "Open in browser when the port binds" back on once.
- **Fixed: a leftover helper could pose as a running project, and refused to be killed.** If a dev server died but left a `workerd` behind, that leftover showed up as though the project were running, on a port like `57018`, and Kill did nothing to it: an orphaned `workerd` ignores a polite shutdown request entirely. Killing now escalates when something refuses to go, and a leftover whose parent process is gone is no longer mistaken for a server you started. Starting a project also clears out anything its last run left behind.
- **Fixed: those helpers also piled up invisibly, two per restart.** Killing a dev server never took them with it, because by then they belong to the system rather than to the server. They kept running and holding ports until you rebooted, and they became briefly visible on every restart, in the moment between the old server dying and the new one binding: restart a project four times and sixteen phantom rows would flash in and out. Killing or restarting a server now clears them out too. If a project has a second dev server still running, nothing is touched, since that server's helpers are still in use.
- **Folders selected in Finder can now be started from the dashboard.** Pressing `⌘N` there opens the picker, which deliberately ignored your Finder selection: the only way to act on folders you had already selected was to leave the dashboard and go back to root search. The picker now shows them at the top under **Selected in Finder**, as one row naming what it would start. Opening the Start command straight from Finder still starts the selection immediately, as before. The difference is who proposed it: arriving from the dashboard you asked to choose, so the selection waits for `↵` instead of spawning on its own. Pressing `↵` starts them without a further "start several folders?" prompt, since the row has already named them; you are still asked about any of them that happen to be running already.
- **The menu bar count now updates the moment a start or restart fails**, instead of waiting up to a minute for its next timed refresh. A restart has already stopped the old server by the time it can fail, so the count had genuinely changed; only successes used to report it.
- **The "Restarted" confirmation now slips away after a couple of seconds**, the way the start confirmation always did, instead of sitting until something replaced it.
- **Cloudflare Worker projects now show a Wrangler tag in the Start picker.** Projects that build with Vite or another bundler keep that tag; the Wrangler label is for projects where Wrangler is the dev server itself.
