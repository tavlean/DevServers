...
## [Dev servers outside the JS world] - {PR_MERGE_DATE}

- **Dev servers outside the JS world now show up.** The list used to be built from processes running out of `node_modules` (plus Bun and the Shopify CLI), so a `python -m http.server` an agent started for you, a Flask or Django app, `rails s`, `php -S`, `go run`, `cargo run` or a `deno run` server was invisible however plainly it was listening. Detection now recognises Python (`http.server`, Flask, Django, uvicorn, FastAPI, gunicorn, MkDocs, Streamlit), Ruby (Rails, Puma, Rack, Jekyll, WEBrick), PHP (`php -S`, Laravel, Symfony), Deno, `go run` / `cargo run` / `dotnet run` binaries, Hugo, Zola, Caddy, and any script that Node, Python, Ruby or PHP is running out of a project folder. Rows carry the tool tag (`http.server`, `Rails`, `Go`) and, where it adds something, a runtime badge (`Python`, `Ruby`, `PHP`, `Deno`), the way Bun already had one. What it deliberately still ignores: daemons, language servers, and anything working out of an app's own folder (`~/.codex`, `~/.cursor`, `~/.vscode`, `~/Library`, system prefixes) or, for bare scripts, the filesystem root, so the list stays a list of dev servers rather than everything that listens. That last rule applies to every kind of server: Codex's visualization servers, which used to surface as projects named after a UUID, no longer appear.
- **Restart works for those projects too, from the dashboard and the menu bar alike.** A server that is not a Node or Bun project is restarted with the command line it was running, in the same folder, even when the folder also carries a package.json for an asset build; for `go run` and `cargo run` that is the launcher's command, not the temporary binary. Node and Bun projects keep restarting through their package.json script as before. A server whose command line cannot be run again (Puma and gunicorn overwrite theirs with a status line) is refused up front with a message saying so, instead of being killed and left down. Kill, Copy URL, Open in Browser and the rest already worked on anything listed.
- **A supervisor and its workers on one port are one row, and go down together.** uvicorn `--reload`, gunicorn and clustered Puma bind the port in a parent and hand the socket to children; only the parent is listed, and killing or restarting it takes the workers with it, so nothing is left holding the port.
- **Only startable folders are remembered.** A folder the extension has no way to start cold (a `python -m http.server` in Documents, a Rails app) is listed while it runs but does not appear later under Start.
