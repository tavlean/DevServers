...
- **Killing from the menu bar confirms itself.** A small system HUD says "Killed <project>" once the process has actually exited (and "Killed both/all N servers" for the per-project kill), instead of the only signal being the menu bar count quietly shrinking a refresh later. The count itself also updates a few seconds sooner after a kill.
