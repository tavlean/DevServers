...
- **Restart works for those projects too.** A project with no package.json script to re-run is restarted with the command line it was running, in the same folder; for `go run` and `cargo run` that is the launcher's command, not the temporary binary. Kill, Copy URL, Open in Browser and the rest already worked on anything listed.
- **A supervisor and its workers on one port are one row.** uvicorn `--reload`, gunicorn and clustered Puma bind the port in a parent and hand the socket to children; only the parent is listed, which is also the one a Kill has to reach.
